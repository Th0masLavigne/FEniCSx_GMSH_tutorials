...
if mesh_comm.rank == model_rank:
    ...
    #===================================================================================
    #                         CREATION OF THE MEDIUM and AIR VOLUME
    #===================================================================================
    """======================CREATION OF THE BODY OF THE AIR-MEDIUM=========================="""
    Chamber = gmsh.model.occ.addCylinder(x1, y1, z1, dx1, dy1, Height+2e-3, r1,tag=-1,angle=np.pi)
    box1       = gmsh.model.occ.addBox(-(16e-3), 0, 0, Length , Width, Height,tag=-1)
    box2       = gmsh.model.occ.addBox(-(16e-3), 0, Height, Length , Width, 2e-3,tag=-1)
    ...
    Fragment_fluid,_  = gmsh.model.occ.fragment([(3,Chamber)],[(3,IWell),(3,Well),(3,box1),(3,E_air),(3,S_air),(3,Pipe_Left),(3,Pipe_Right),(3,box2)])
    Total_fluid,_= gmsh.model.occ.fuse([Fragment_fluid[0]],Fragment_fluid[1:-3])
    # Remove the extra volumes required for the good "cylinder"
    gmsh.model.occ.remove(Fragment_fluid[-2:],recursive=True)
    # 
    """========================= REDUCTION OF THE NUMBER OF VOLUMES=========================="""
    Air_Med_separator= gmsh.model.occ.addBox(-(5e-2), -5e-2, -5e-2, 2*5e-2 , 2*5e-2, 5e-2+Height/2,tag=-1)
    Fragment_fluid,_ = gmsh.model.occ.intersect(Total_fluid,[(3,Air_Med_separator)],removeTool=True,removeObject=False)
    # 
    #===================================================================================
    ...
    Obs_up = gmsh.model.occ.addCylinder(x5, y5, (8e-3), dx5, dy5, dz5+7e-2, r5,tag=-1,angle=np.pi)
    Obs_down = gmsh.model.occ.addCylinder(x5, y5, -2.5e-3, dx5, dy5, dz5, r5,tag=-1,angle=np.pi)
    plastic,_=gmsh.model.occ.cut(plastic, [(3, Obs_down)], removeTool=True, removeObject=True)  
    plastic,_=gmsh.model.occ.cut(plastic,[(3,Obs_up)], removeTool=True, removeObject=True)  
    # """========================CREATION OF CYLINDRICAL COVER GLASSES========================="""
    Glass_up     = gmsh.model.occ.addCylinder(x6, y6, 8e-3, dx6, dy6, dz6, r6, tag=15, angle=np.pi)
    Glass_down   = gmsh.model.occ.addCylinder(x6, y6, -2e-3, dx6, dy6, -dz6, r6, tag=16, angle=np.pi)
    gmsh.model.occ.synchronize()
    gmsh.model.occ.removeAllDuplicates()
    gmsh.model.occ.synchronize()
    print(volumes)
    ...
    # 
    tdim =3 
    gmsh.model.addPhysicalGroup(tdim, [23,17,18], 1)
    gmsh.model.setPhysicalName(tdim, 1, 'air')
    gmsh.model.addPhysicalGroup(tdim, [22], 2)
    gmsh.model.setPhysicalName(tdim, 2, 'fluid')
    gmsh.model.addPhysicalGroup(tdim, [24], 3)
    gmsh.model.setPhysicalName(tdim, 3, 'plastic')
    gmsh.model.addPhysicalGroup(tdim, [15,16], 4)
