...
This example aims in providing an example of a 3D hyper-elastic beam of dimensions 40 x 1 x 1 introducing penalty contact with a plane of equation z=-4.
...
```{math}
\mathbf{\sigma}(\mathbf{u}) = \frac{\mathrm{d}\psi}{\mathrm{d}F}, \text{ with } \psi = \frac{\mu}{2} (\mathrm{tr}(F^T F)-3)-\mu\ln(\mathrm{det}F)+\frac{\lambda}{2} (\ln(\mathrm{det}F))^2
```
```python
# Constitutive Law
